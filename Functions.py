...
def denoising(noisy_image, learning_image, window_shape, window_step, sigma, learning_ratio=0.1, ksvd_iter=1):
    ...
    # 5. (by me) save some temporary I want
    handle = open(r".\dictionary.npy","wb")
    np.save(handle,basis_final)
    np.save(handle, sparse_final)
    np.save(handle,padded_denoised_image)
    np.save(handle)
